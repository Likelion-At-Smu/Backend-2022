...
def detail(requset, post_id):
        # get_object_or_404 -> pk 값을 이용해 특정 객체 하나만 가져오기 위해 사용
...
def new_comment(request, post_id):
    ...
    if filled_form.is_valid():
        finshed_form = filled_form.save(commit=False) # 저장하지 않고 대기 
        finshed_form.post = get_object_or_404(Post,pk=post_id) # 게시물에 해당하는 댓글을 담아줌
...
def freepostcreate(request):
    if request.method == 'POST' or request.method == 'FILES':
        ...
        if form.is_valid():
            unfinished = form.save(commit=False) # 저장하지 않고 대기
            # user 객체를 author에 추가
